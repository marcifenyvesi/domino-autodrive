...
| 3 | **Nothing ships un-challenged.** Every research doc, design, and task set is reviewed by **N independent blind rounds** (fresh subagent each, blind to its siblings), adjudicated into one cited verdict per issue, and must **converge** (a post-foldback blind round yields zero `CONFIRMED`) before it advances. | The `/challenge --round` gate + the `challenge-adjudicate` skill: independent rounds → union matrix → verdicts settled against the tree → single foldback off `CONFIRMED`, with a witness-count calibration table. Independence + adjudication replaces self-referential re-passing, so review can't become a token sink. | [`commands/challenge.md`](commands/challenge.md), [`skills/challenge-adjudicate/SKILL.md`](skills/challenge-adjudicate/SKILL.md) |
